package com.flexnet.presentation.feature.component.jsonviewer

import com.evdayapps.jsonviewer.JsonViewerStyles
...
class JsonParser(
    ...
    fun parse(strJson: String): List<JsonItem> {
        ...
        if (data != null) {
            when (data) {
                is JSONObject,
                is JSONArray,
                -> return parseElement(element = data)
            }
        }
    ...
    fun safeParse(strJson: String): List<JsonItem> {
        return try {
            parse(strJson)
        } catch (e: Exception) {
            emptyList()
